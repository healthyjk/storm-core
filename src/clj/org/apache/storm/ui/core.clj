...
(ns org.apache.storm.ui.core
  ...
  (:import [org.apache.storm.generated AuthorizationException ProfileRequest ProfileAction NodeInfo GpuInfo GpuUtilizationInfo])
   ...
   {"supervisors"
    (for [^SupervisorSummary s summs]
      {"id" (.get_supervisor_id s)
       ...
       "gpuCount" (count (.get_gpu_infos s))
       "gpuInfos" (for [^GpuInfo gpu-info (.get_gpu_infos s)]
                    {"id" (.get_id gpu-info)
                     "name" (.get_name gpu-info)
                     "driverVersion" (.get_driver_version gpu-info)
                     "computeCapability" (str (str (.get_major gpu-info)) "." (str (.get_minor gpu-info)))
                     "totalMemSize" (.get_total_mem_size gpu-info)
                     "multiProcessorCount" (.get_multi_processor_count gpu-info)
                     "cudaCoresPerMultiProc" (.get_cuda_cores_per_multiproc gpu-info)
                     "gpuClockRate" (.get_gpu_clock_rate gpu-info)
                     "memClockRate" (.get_mem_clock_rate gpu-info)
                     "memBusWdith" (.get_mem_bus_width gpu-info)
                     "deviceOverlap" (.is_device_overlap gpu-info)
                     "asyncEngineCount" (.get_async_engine_count gpu-info)
                     })
       "gpuUtilization" (for [^GpuUtilizationInfo gpu-util-info (.get_gpu_util_infos s)]
                          {"id" (.get_id gpu-util-info)
                           "gpuUtil" (.get_gpu_util gpu-util-info)
                           "totalMem" (.get_total_mem gpu-util-info)
                           "usedMem" (.get_used_mem gpu-util-info)
                           "processCount" (.get_process_count gpu-util-info)
                           })
       })
